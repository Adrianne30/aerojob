...
async function scrapeAviationJobs() {
  ...
  // ✅ Use MyNimo as target
  const targetURL = 'https://www.mynimo.com/jobs/philippines?q=aircraft';
  ...
  // ✅ Adjust selectors for MyNimo layout
  $('.job-post').each((_, el) => {
    const title = $(el).find('.position-title a').text().trim();
    const company = $(el).find('.company-name').text().trim();
    const location = $(el).find('.job-location').text().trim();
    const link = 'https://www.mynimo.com' + ($(el).find('.position-title a').attr('href') || '');
